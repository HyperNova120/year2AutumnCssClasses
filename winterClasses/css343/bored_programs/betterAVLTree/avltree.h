...
namespace AVLTreeInternals
{
    ...
    template <typename T, typename TComparison>
    inline bool AVLTreeBase<T, TComparison>::insert(T obj)
    {
        if (root_ == nullptr)
        {
        ...
        Node *reader = root_;
        bool moveLeft = TCompare(NodeData<T>{obj}, *reader->nodeData);
        while (!TEqual(reader->nodeData->data_, obj) && ((moveLeft) ? reader->left_ : reader->right_) != nullptr)
        {
            ...
            moveLeft = TCompare(NodeData<T>{obj}, *reader->nodeData);
        }
    ...
    template <typename T, typename TComparison>
    inline bool AVLTreeBase<T, TComparison>::contains(T obj) const
    {
        Node *reader = root_;
    ...
    inline bool AVLTreeBase<T, TComparison>::TEqual(const T &A, const T &B) const
    {
        NodeData<T> ADat = {A};
        NodeData<T> BDat = {B};
        return !(TCompare(ADat, BDat)) && !(TCompare(BDat, ADat));
    }
} // AVLTreeBase
